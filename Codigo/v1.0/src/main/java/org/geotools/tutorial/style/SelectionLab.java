...
public void displayShapefile(File file) throws Exception {
     ...
     * of CursorTool.
     */
    btn.addActionListener(e ->mapFrame.getMapPane().setCursorTool(new CursorTool() {@Override
...
//    System.out.println("X getX: "+ev.getX()+" Y getY: "+ev.getY());
//    System.out.println("X getXOnScreen: "+ev.getXOnScreen()+" Y getYOnScreen: "+ev.getYOnScreen());
    
    
    //AQUI AGREGARE CODIGO SOBRE EL TUTORIAL DE GEOTOOLS
    
    /**
     * Aqui agregar antes (o donde corresponda) preguntar al hacer click si desea agregar el punto para recien preseguir y que le ponga un nombre
     */
    ...
    try {
        ...
        contenido=contenido+"x:"+String.valueOf(ev.getWorldPos().getX())+"y:"+String.valueOf(ev.getWorldPos().getY())+"\n";
    ...
    Rectangle screenRect = new Rectangle(screenPos.x - 2, screenPos.y - 2, 5, 5); //AQUI OBTIENE LATITUD Y LONGITUD AHORA VER SI ES ESO O SON PIXELES PRIMERO
